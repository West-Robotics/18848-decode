package org.firstinspires.ftc.teamcode.robertMkII.subsystems

import com.qualcomm.robotcore.hardware.DcMotor
import com.qualcomm.robotcore.hardware.DcMotorSimple
import com.qualcomm.robotcore.hardware.HardwareMap
import org.firstinspires.ftc.teamcode.robertMkII.controlEffort
import org.firstinspires.ftc.teamcode.robertMkII.hardware.NgEncoder
import org.firstinspires.ftc.teamcode.robertMkII.hardware.NgMotor

class Extender(hardwareMap: HardwareMap) {
    val kp = 1.5

    private val motor = NgMotor(hardwareMap, "extender", DcMotorSimple.Direction.REVERSE, DcMotor.ZeroPowerBehavior.BRAKE)
    private val encoder = NgEncoder(hardwareMap, "extender", DcMotorSimple.Direction.REVERSE, 537.7)

    fun setEffort(power: Double) {
        motor.effort = power
    }

    fun getEffort() = motor.effort

    fun resetEncoder(ticks: Int = 0) {
        encoder.reset(ticks)
    }

    fun write() {
        motor.write()
    }

    fun runToPos(targetPos: Double, currentPos:Double) {
        val effort = controlEffort(targetPos, currentPos, kp)
        setEffort(effort)
    }

    fun getTicks() = encoder.ticks
    fun getRevs() = encoder.revs
    fun getVelocity() = encoder.tickV
    fun getCurrent() = motor.getCurrent()
}